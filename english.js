...
// async function gg (){
//   let i = await client.getEntries({ content_type: "english"});
//   console.log(i.items[0].fields);
// }
// gg();
// State Handler
...
const UiCtr = (() => {
  const uiElements = {
    ...
    quizreading: document.querySelector('.quiz-reading')
  };
  return {
    showScore: (e) => {
    ...
    displayQuestion: () => {
      const [c,q,...options] = State.getQuestions()[State.getPosition()];
      let labels = Array.from(document.querySelectorAll('.option'));
      uiElements.quizreading.textContent = c;
...
// show next question
  function showNextQuestion(e) {
    ...
    UiCtr.getUiElements().questionNumber.textContent = State.getPosition() + 1;
    UiCtr.displayQuestion();
    // document.querySelector('.section1').scrollIntoView(top);
  ...
  async function beforeAnything() {
    let  q = await client.getEntries({ content_type: "english"});
    let p = await q.items[0].fields.englishtest;
